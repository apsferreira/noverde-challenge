...
def insert(self, request):
    ...
    loan = Loan(
        ...
        amount_asked=Decimal(request['amount']),
        terms_asked=request['terms'],
        income=Decimal(request['income']),
    )
...
def refuse(self, loan_id, refused_policy):
...
def approve(self, loan_id, terms_approved, amount_approved):
    loan_obj = Loan.objects.get(id=loan_id)
...
def _validate_data(self, data):
    ...
    elif Decimal(amount_asked) < self.AMOUNT_MIN or self.AMOUNT_MAX < Decimal(amount_asked):
